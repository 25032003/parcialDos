...
public class Menú {
    ...
    double total = 0;
    double precioPequena = 50;
    double precioMediana = 75;
    double precioGrande = 100;
    ...
    public Menú() {
        ...
        ingredientesPorPizza.put("Pizza de Tacos", Arrays.asList(
                new Ingrediente("Salsa Q.10.00", 10.00),
                new Ingrediente("Taco Q15.00", 15.00),
                new Ingrediente("Carne Q.20.00", 20.00),
                new Ingrediente("Queso Q.15.00", 15.00),
                new Ingrediente("Cebolla Q.10.00", 10.00)
        ));

        ingredientesPorPizza.put("Pizza de Sushi", Arrays.asList(
                new Ingrediente("Salsa de soja Q.10.00", 10.00),
                new Ingrediente("Sushi Q.15.00", 15.00),
                new Ingrediente("Arroz Q8.00", 8.00),
                new Ingrediente("Alga Q.5.50", 5.50),
                new Ingrediente("Pescado Q.15.00", 15.00)
        ));

        ingredientesPorPizza.put("Pizza de Chocolate", Arrays.asList(
                new Ingrediente("Chocolate Q.8", 8.0),
                new Ingrediente("Fresa Q.3.00", 3.00),
                new Ingrediente("Crema Q.5.00", 5.00),
                new Ingrediente("Queso Q.10.00", 10.00)
                ));

        ingredientesPorPizza.put("Pizza de Espárragos y Camarones", Arrays.asList(
                new Ingrediente("Espárragos Q.10.00", 10.00),
                new Ingrediente("Camarones Q.5.00", 5.00),
                new Ingrediente("Queso Q.7.00", 7.00),
                new Ingrediente("Cebolla Q.5.50", 5.50)
                ));
        ...
        botonpreparar.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (textField1 == null || textField1.getText().isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Escriba un Nombre para la Pizza.");
                    ...
                    if (pizzaSeleccionada == null || pizzaSeleccionada.isEmpty()) {
                        JOptionPane.showMessageDialog(null, "Tienes que seleccionar una pizza de primero.");
                        return; // Sale del ActionListener, ya que no se puede preparar la pizza sin una seleccionada.
                    }

                // Verificar si se ha seleccionado un tamaño de pizza
                if (precioseleccionado == 0.0) {
                    JOptionPane.showMessageDialog(null, "Seleccione el tamaño de una pizza primero.");
                    return;
                }
                    ...
                    mensajePreparacion.append("La pizza que esta preparando es de:  ").append(pizzaSeleccionada).append(":\n");
